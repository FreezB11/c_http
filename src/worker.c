...
void *worker_thread(void *arg) {
    ...
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(cpu % (int)sysconf(_SC_NPROCESSORS_ONLN), &cpuset);
    pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);

    int epoll_fd = epoll_create1(EPOLL_CLOEXEC);
    if (epoll_fd < 0) { perror("epoll_create1"); return NULL; }

    /* Each worker owns its own listen socket (SO_REUSEPORT) */
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (fd == listen_fd) {
                ...
                while (1) {
                    ...
                    int cfd = accept4(listen_fd, (struct sockaddr *)&cli,
                                      &clen, SOCK_NONBLOCK);
                    if (cfd < 0) break;
                    ...
                    /* ✔ Only EPOLLIN on new connection */
            ...
            /* ✔ Re-arm with correct event for current state */
            ev.data.ptr = ctx;
